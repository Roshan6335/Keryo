...
export default async function handler(req, res) {
    ...
    if (planError) {
        console.error('[activate-plan] user_plans write failed:', planError.message);
        return res.status(500).json({ error: 'Could not activate plan. Contact support.' });
    }
    ...
    if (profileError) {
        console.error('[activate-plan] profiles sync failed:', profileError.message);
        return res.status(500).json({ error: 'Failed to sync profile. Contact support.' });
    }

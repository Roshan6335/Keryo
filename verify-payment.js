...
export default async function handler(req, res) {
    ...
    // Write to user_plans — check for error
    ...
    if (planError) {
        console.error('[verify-payment] user_plans write failed:', planError.message);
        return res.status(500).json({ error: 'Failed to record subscription. Contact support with payment ID: ' + razorpay_payment_id });
    }

    // Sync to profiles — check for error
    ...
    if (profileError) {
        console.error('[verify-payment] profiles sync failed:', profileError.message);
        return res.status(500).json({ error: 'Failed to sync subscription to profile. Contact support.' });
    }

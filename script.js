...
window.handleGoogleCredential=function(resp){
    ...
    try{
        ...
        saveSession(); // persist immediately so refresh does not log out
...
function setSidebar(collapsed){
    ...
    // Remove then re-add class so transition fires cleanly (prevents glitch)
    sb.classList.remove('collapsed');
    void sb.offsetWidth; // force reflow to reset transition
    if(collapsed){
...
/* ======================================================
   CHECKOUT MODAL -- coupon + price breakdown + pay
====================================================== */

var _checkoutPlan   = null;
var _checkoutCoupon = null;

function openCheckoutModal(plan){
    if(!currentUser){toast('Please sign in first','error');return;}
    _checkoutPlan   = plan;
    _checkoutCoupon = null;
    var planInfo=(window.CONFIG&&CONFIG.PLANS&&CONFIG.PLANS[plan])||{label:plan,price:0};
    var nameEl=el('co-plan-name');
    if(nameEl) nameEl.textContent='Keryo '+(planInfo.label||plan);
    var inp=el('co-coupon-input'), fb=el('co-coupon-feedback'), ab=el('co-coupon-apply-btn');
    if(inp){inp.value='';inp.className='coupon-input';}
    if(fb){fb.style.display='none';fb.textContent='';fb.className='coupon-feedback';}
    if(ab){ab.disabled=false;ab.textContent='Apply';}
    _updateCheckoutPrices(planInfo.price||0, 0);
    var payBtn=el('co-pay-btn');
    if(payBtn){payBtn.disabled=false;payBtn.innerHTML='Pay &#8377;'+(planInfo.price||0)+' &rarr;';}
    var modal=el('checkout-modal');
    if(modal) modal.style.display='flex';
}

function _updateCheckoutPrices(origPrice, discountPct){
    var discounted  = discountPct>0 ? Math.round(origPrice*(1-(discountPct/100))) : origPrice;
    var discountAmt = origPrice - discounted;
    var Rs = '&#8377;';
    var origEl=el('co-original-price'), discRow=el('co-discount-row');
    var discLabel=el('co-discount-label'), discAmt=el('co-discount-amt'), totalEl=el('co-total-price');
    if(origEl)   origEl.innerHTML  = Rs+origPrice;
    if(totalEl)  totalEl.innerHTML = Rs+discounted;
    if(discountPct>0){
        if(discRow)   discRow.style.display='flex';
        if(discLabel) discLabel.textContent='Coupon ('+discountPct+'% off)';
        if(discAmt)   discAmt.innerHTML='-'+Rs+discountAmt;
    } else {
        if(discRow) discRow.style.display='none';
    }
    var payBtn=el('co-pay-btn');
    if(payBtn&&!payBtn.disabled){
        if(discounted===0){ payBtn.textContent='Activate for FREE'; }
        else { payBtn.innerHTML='Pay '+Rs+'<span id="co-pay-amount">'+discounted+'</span> &rarr;'; }
    }
}

function _closeCheckoutModal(){
    var m=el('checkout-modal');
    if(m) m.style.display='none';
    _checkoutPlan=null; _checkoutCoupon=null;
}

// Checkout coupon apply
(function(){
    var ab=el('co-coupon-apply-btn');
    if(!ab) return;
    ab.addEventListener('click', async function(){
        if(!_checkoutPlan) return;
        var inp=el('co-coupon-input');
        var code=(inp&&inp.value||'').trim();
        var fb=el('co-coupon-feedback');
        if(!code){
            if(fb){fb.textContent='Please enter a coupon code.';fb.className='coupon-feedback cf-error';fb.style.display='block';}
            return;
        }
        ...
        try{
            var resp=await fetch('/api/create-order',{
                ...
                body:JSON.stringify({plan:_checkoutPlan, userId:currentUser?currentUser.sub:'guest_probe', coupon:code, probeOnly:true})
            });
            var data=await resp.json();
            var pi=(window.CONFIG&&CONFIG.PLANS&&CONFIG.PLANS[_checkoutPlan])||{price:0};
            if(!resp.ok){
                inp.className='coupon-input coupon-invalid';
                if(fb){fb.textContent=data.error||'Invalid coupon code.';fb.className='coupon-feedback cf-error';fb.style.display='block';}
                _checkoutCoupon=null;
                _updateCheckoutPrices(pi.price||0, 0);
            } else {
                inp.className='coupon-input coupon-valid';
                _checkoutCoupon={code:code, discountPct:data.discountPct||0, free:data.free||false};
                _updateCheckoutPrices(pi.price||0, data.discountPct||0);
                var msg=data.free?'100% off! Plan is FREE!':''+data.discountPct+'% off applied!';
                if(fb){fb.textContent=msg;fb.className='coupon-feedback cf-success';fb.style.display='block';}
            }
        }catch(_){
            if(fb){fb.textContent='Could not validate. Try again.';fb.className='coupon-feedback cf-error';fb.style.display='block';}
            _checkoutCoupon=null;
        }finally{
            ab.disabled=false; ab.textContent='Apply';
        }
    });
})();
...
// Checkout pay button
(function(){
    var pb=el('co-pay-btn');
    if(!pb) return;
    pb.addEventListener('click', async function(){ if(_checkoutPlan) await startPayment(_checkoutPlan); });
})();

// Checkout modal close
(function(){
    var cb=el('checkout-modal-close'), ov=el('checkout-modal');
    if(cb) cb.addEventListener('click',_closeCheckoutModal);
    if(ov) ov.addEventListener('click',function(e){if(e.target===this)_closeCheckoutModal();});
})();
...
async function startPayment(plan){
    if(!currentUser){toast('Please sign in first','error');return;}
    var planInfo=(window.CONFIG&&CONFIG.PLANS&&CONFIG.PLANS[plan])||{label:plan,price:0};
    var couponCode=(_checkoutCoupon&&_checkoutCoupon.code)||'';
    var payBtn=el('co-pay-btn');
    ...
    try{
        ...
        if(!resp.ok) throw new Error(orderData.error||'Something went wrong. Please try again.');
        if(orderData.free){
            _closeCheckoutModal();
        ...
        var discAmt=Math.round(orderData.amount/100);
        var options={
            ...
            modal:{
                ondismiss:function(){
                    if(payBtn){payBtn.disabled=false;payBtn.innerHTML='Pay &#8377;'+discAmt+' &rarr;';}
                }
            },
        ...
        if(payBtn){
            ...
            var planInfo2=(window.CONFIG&&CONFIG.PLANS&&CONFIG.PLANS[plan])||{price:0};
            var couponDisc=(_checkoutCoupon&&_checkoutCoupon.discountPct)||0;
            var origPrice2=planInfo2.price||0;
            var discPrice2=couponDisc>0?Math.round(origPrice2*(1-couponDisc/100)):origPrice2;
            payBtn.innerHTML='Pay &#8377;'+discPrice2+' &rarr;';
        }
        updateAllPlanUI();
    }
}
...
/* ──────────────────────────────────────────────────────────
   VERIFY PAYMENT — called after Razorpay success callback
   Sends payment IDs to backend for signature verification
   and database write. Shows success animation on confirm.
────────────────────────────────────────────────────────── */
async function verifyPayment(plan, rzpResp) {
    try {
        ...
        toast('Verification failed: ' + err.message, 'error', 4000);
        updateAllPlanUI();
    }
}

/* ──────────────────────────────────────────────────────────
   ACTIVATE FREE PLAN — called when coupon gives 100% off
   Hits /api/activate-plan endpoint to write to database.
────────────────────────────────────────────────────────── */
async function activateFreePlan(plan, coupon) {
    try {
        ...
        toast('Activation failed: ' + err.message, 'error', 4000);
        updateAllPlanUI();
    }
}
...
// Fullscreen button for image viewer
(function(){
    var fsBtn=el('img-viewer-fullscreen');
    if(fsBtn){
        fsBtn.addEventListener('click',function(){
            var imgEl=el('img-viewer-src');
            if(!imgEl) return;
        ...
        document.addEventListener('fullscreenchange',function(){
            ...
            if(fsBtn){
                ...
                fsBtn.innerHTML=isFs
                    ?'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M8 3v3a2 2 0 01-2 2H3m18 0h-3a2 2 0 01-2-2V3m0 18v-3a2 2 0 012-2h3M3 16h3a2 2 0 012 2v3"/></svg>'
...
// Escape key closes image viewer
document.addEventListener('keydown',function(e){
    if(e.key==='Escape'){
        var iv=el('img-viewer-modal');
        if(iv&&iv.style.display==='flex') iv.style.display='none';
    }
});
